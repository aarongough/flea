...
describe Flea::Interpreter do
  ...
  describe ".new" do
    it "returns an Interpreter" do
      expect(Interpreter.new).to be_a Interpreter
    end
  ...
  describe "#evaluate" do
    ...
    let(:interpreter) { Interpreter.new(base_environment: environment, load_standard_library: false) }
    ...
    it "calls a native function" do
      allow(environment).to receive(:find).with(:foo).and_return(Proc.new {|a,b| "bar"})
